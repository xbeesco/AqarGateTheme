...
jQuery(document).ready( function($) {
    ...
    $("#brokerageContractNumber").on("input", function() {
        if ( $(this).val() !== "" && $(this).val().length === 10 ) {
            $("#all-info").fadeIn(1000);
            houzezOSM.invalidateSize();
        } else {
            $("#all-info").fadeOut(1000);
        }
    });

    const countyState = $('#countyState');
    const city = $('#city');

    city.change(function() {
        if (countyState.val() !== '') {
            let q = 'المملكة العربية السعودية, ' + countyState.find('option:selected').text() + ', ' + city.find('option:selected').text();
            console.log(q);

          // Execute your code block here
          $.get("https://nominatim.openstreetmap.org/search.php?q="+ q +"&format=json&polygon=1", function(data) {
                console.log(data);
                if (data.length > 0) {
                var property_lat = data[0].lat;
                var property_long = data[0].lon;
                console.log('lat : ' + property_lat);
                console.log('lon : ' + property_long);
                houzez_osm_marker_position(property_lat, property_long);
                } else {
