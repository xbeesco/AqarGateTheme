...
$user_prop_request = $ac_crm = $ac_insight = $ac_profile = $ac_props = $ac_add_prop = $ac_fav = $ac_search = $ac_invoices = $ac_msgs = $ac_mem = $ac_gdpr = $ac_agents = '';
if( is_page_template( 'template/user_dashboard_profile.php' ) ) {
    ...
    $user_prop_request = 'class=active';
}
    ...
    <div class="navbar-logged-in-wrap navbar">
        ...
        <ul class="logged-in-nav dropdown-menu">
            ...
            if( !empty( $user_property_request ) ) {
                echo '<li class="side-menu-item">
                        <a '.esc_attr( $user_prop_request ).' href="'.esc_url($user_property_request).'">
                            <i class="houzez-icon icon-building-cloudy mr-2"></i> طلباتي العقارية
                        </a>
                    </li>';
            }
            ...
            if( !empty( $add_property_request ) ) {
                echo '<li class="side-menu-item">
                        <a '.esc_attr( $user_prop_request ).' href="'.esc_url($add_property_request).'">
                            <i class="houzez-icon icon-add-circle mr-2"></i>اضافة طلب عقاري
                        </a>
                    </li>';
            }
