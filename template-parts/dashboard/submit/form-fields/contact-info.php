...
$fave_author_agent_id = get_user_meta(get_current_user_id(), 'fave_author_agent_id', true); 
$agent_agency_id = get_user_meta(get_current_user_id(), 'fave_author_agency_id', true);
$agents_arg = [
	'post_type' => 'houzez_agent', 
	'posts_per_page' => -1, 
	'suppress_filters' => 0
];
if( $fave_author_agent_id > 1 ) {
	$agents_arg['p'] = $fave_author_agent_id;
}
$agents_posts = get_posts($agents_arg);
$agency_arg = [
	'post_type' => 'houzez_agency', 
	'posts_per_page' => -1, 
	'suppress_filters' => 0
];
if( $agent_agency_id > 1 ) {
	$agency_arg['p'] = $agent_agency_id;
}
$agency_posts = get_posts($agency_arg);
...
<div class="form-group">
	...
	<label class="control control--checkbox " style="display:none;">
	...
	<?php if ( ! houzez_is_agency() && empty($agent_agency_id) ) { ?>
	        	...
	        	if (!empty($agents_posts)) { 
	                ...
	                foreach ($agents_posts as $agent_post) {
	                    ...
	                    if(houzez_edit_property()) {
							...
							if( (int) $fave_author_agent_id === (int) $agent_post->ID ) {
								$output .= ' selected';
							}
	                    }
	...
	<?php  } ?>
	<?php if ( ! houzez_is_agency() && ! empty($agent_agency_id) ) { $agent_display_option = 'agency_info'; ?>
	<label class="control control--checkbox">
            ...
            if (!empty($agency_posts)) {
                ...
                foreach ($agency_posts as $agency) {
                    ...
                    if(houzez_edit_property()) {
						...
						if ( ! houzez_is_agency() ) { 
							if( (int) $agent_agency_id === (int) $agency->ID ) {
								$output .= ' selected';
							}
						}
                    }
