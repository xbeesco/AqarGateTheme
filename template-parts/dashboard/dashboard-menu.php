...
$add_property_request = houzez_get_template_link_2('property-request-page.php');
...
$ac_user_property = $parent_crm = $parent_props = $parent_agents = $ac_crm = $ac_insight = $ac_profile = $ac_props = $ac_add_prop = $ac_fav = $ac_search = $ac_invoices = $ac_msgs = $ac_mem = $ac_gdpr = '';
...
} elseif ( is_page_template( 'template/user_property_request.php' ) ) {
    $ac_user_property = 'class=active';	
...
$ac_approved = $ac_pending = $ac_expired = $ac_disapproved = $ac_all = $ac_draft = $ac_on_hold = $ac_agents = $ac_agent_new = '';
...
<ul class="side-menu list-unstyled">
	...
	if ( !is_user_logged_in() ) {
					...
					<ul class="side-menu-dropdown list-unstyled">
						...
						<li class="side-menu-item">
							<a '.esc_attr( $ac_on_hold ).' href="'.esc_url($on_hold).'">
								<i class="houzez-icon icon-arrow-right-1"></i> '.houzez_option('dsh_hold', 'On hold').'
							</a>
						</li>
						<li class="side-menu-item">
							<a '.esc_attr( $ac_disapproved ).' href="'.esc_url($disapproved).'">
								<i class="houzez-icon icon-arrow-right-1"></i> '.houzez_option('dsh_disapproved', 'Disapproved').'
							</a>
						</li>
					</ul>
				</li>';
		...
		if( !empty( $add_property_request )  ) {
			echo '<li class="side-menu-item">
					<a '.esc_attr( $ac_add_prop ).' href="'.esc_url($add_property_request).'">
						<i class="houzez-icon icon-add-circle mr-2"></i> اضافة طلب عام
					</a>
				</li>';
	    }

		if( !empty( $user_property_request ) ) {
			echo '<li class="side-menu-item">
					<a '.esc_attr( $ac_user_property ).' href="'.esc_url($user_property_request).'">
						<i class="houzez-icon icon-building-cloudy mr-2"></i> الطلبات العامة
					</a>
				</li>';
	    }
